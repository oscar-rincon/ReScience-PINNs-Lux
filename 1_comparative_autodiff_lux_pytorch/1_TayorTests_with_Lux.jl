using Lux, Random, TaylorDiff, Plots, Optimisers, Zygote, ComponentArrays, Printf
...
train_state = train_model!(train_state, vjp_rule, (input_data, true_output), 7_000)
...
gradient_taylor = TaylorDiff.derivative(neural_network, input_data, Float32.(ones(size(input_data))), Val(2))
gradient_target = TaylorDiff.derivative(target_function, input_data, Float32.(ones(size(input_data))), Val(2))
